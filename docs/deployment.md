...
Provisioning initializes an empty database from the bundled, hash-verified `src/cloudflare/schema.sql`. Existing databases must contain `crate_schema` with version 2, 3, 4, 5 or 6. Versions 2–5 first run `schema-v6.sql` as one transaction under the deployment fence, then apply the current schema. The migration copies persisted portable keys and preserves original paths, file pointers, history and receipts. It adds nullable ownership fields to older cleanup records and advances the sync snapshot baseline for nonempty vaults. Devices take a fresh snapshot once so pagination cannot cross the ordering change. An existing open import keeps its token and progress. Other schemas are rejected before mutation.

The schema-6 file table uses the portable path as its primary key. The migration has a one-time D1 write cost and does not rewrite R2 contents. New vaults start with the cheaper layout. Reapplying schema 6 does not copy file rows again. Do not run the fresh `schema.sql` alone against an older file inventory, and do not downgrade the schema marker to roll back.

Install the matching Worker before releasing the deployment fence. If migration succeeds but Worker deployment fails, the fence stays held even after a definite API rejection. Automatic recovery must not unlock a migrated database while the old Worker is live. Finish the matching deployment using the recorded build/fingerprint before releasing that fence. Paired recovery tooling accepts schemas 2–6 and migrates older archives in isolated restore targets while preserving the original archive. See the [compatibility matrix and rollback policy](compatibility.md).
...
For an interrupted **Delete server** operation, reconnect to the network and select **Crate settings → Advanced server actions → Resume server deletion**. After checking resource ownership and the exact retirement Worker, Crate can atomically replace a deletion lock at a confirmed retirement/file/bucket deletion checkpoint, or at an interrupted `deleteR2Object` step. A late file deletion is compatible with permanently removing that same server; Crate re-lists its bucket and never rebuilds these resources. This also supports older object-deletion checkpoints that omit the object key. The replacement uses the exact inspected row, so the previous client cannot checkpoint or dispatch another step. Uncertain Worker publication, bucket deletion, and reset operations still require review. Do not manually recreate resources under the deleted server’s old names.
