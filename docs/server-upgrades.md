...
The provisioner creates and verifies a paired D1/R2 checkpoint before applying Reading's migration. Database write guards remain active until the new deployment is verified, and the original checkpoint is retained across retries. Self-hosted updates require a stopped server and a verified backup through the explicit upgrade command. Recovery uses the same manifest and SQL against an isolated copy and preserves the original archive bytes. See the [Reading testing guide](read-it-later-testing.md) for commands.
