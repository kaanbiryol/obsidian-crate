...
R2 stores immutable bytes under opaque object keys. D1 commits `files`, changelog, retained versions, and Markdown projection intent together. An uncertain database response never authorizes deletion of staged bytes. Each upload records a 24-hour publication lease before writing R2. Cleanup claims expired leases and checks live and retained references before removing objects; claimed uploads cannot publish afterward. A one-time resumable scan covers legacy objects, then bucket-wide scans stop. Replaced/deleted versions are retained for 30 days.
