...
| D1 | `crate_schema = 6` | Empty databases initialize; schema 2/3/4/5 inventories migrate atomically under a deployment fence; other versions are rejected. Joining an existing deployment does not migrate or upload code. |
...
| Paired recovery archive | Archive format 1, schemas 2/3/4/5/6 | Verify source bytes without migration; isolated restore upgrades supported schema 2/3/4/5 to 6 while preserving source archive bytes, receipts and observations. |
...
The schema 2/3/4/5-to-6 upgrade replaces the current-file table inside one transaction and preserves persisted portable keys and original spelling. Keep the deployment fence held until the matching Worker is live. The old Worker must not resume mutations against the new primary key. The migration also expires pre-upgrade snapshots so pagination cannot silently cross the ordering change.
...
Schema 6 replaces the two live-file indexes with a portable-path primary key and owned cleanup intents. Original filenames and wire file identities are unchanged. Apply `schema-v6.sql` once to a supported older database, then `schema.sql`, through the provisioner while the deployment fence is held. An interrupted migration/deployment must retain the fence until the matching Worker is installed. Do not apply the fresh schema alone to an older inventory.
