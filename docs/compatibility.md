...
| Local sync checkpoints | Checkpoint envelope version 2 with generation, server authority, rename dependencies and settled upload IDs | Read supported version-1 and version-2 checkpoints and write version 2. Select the newest valid main/tmp generation; authority mismatch or unsupported state stops sync. Reconfiguration verifies recovery copies before invalidating both files. The remote manifest is a separate version-1 format. |
