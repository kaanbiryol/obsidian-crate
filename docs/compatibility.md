...
| D1 | `crate_schema = 1` | First release baseline; empty databases initialize and current databases receive no implicit DDL. The migration registry is empty; unsupported versions are rejected. Joining an existing deployment does not migrate or upload code. |
...
| Shared history checkpoints | R2 inventory/index format 1; `shared-history-checkpoints-v1` | Protocol 11 adds shared checkpoints with 20-entry/30-day limits; D1 remains schema 1. Older servers retain local-only fallback. Existing local checkpoints are not advertised as shared states. |
...
The first release has no historical schema migrations. Future migrations must preserve file hashes and object references; a refreshed sync listing must not be confused with re-uploading unchanged contents. See the [server upgrade contract](server-upgrades.md).
