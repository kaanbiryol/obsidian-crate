...
| --- | --- | --- |
| API writes | `X-Crate-Protocol: 7`; current and oldest compatible are both 7 | Missing, older or future protocols receive 428 before mutations. Authenticated read endpoints remain available at the HTTP layer; an older application's decoder may still require an update. |
...
| Reminder parsing | Parser version 8 | List caches are disposable. Durable source verification is rebuilt from current R2 revisions before queued or installed notifications can deliver. |
...
| Local sync checkpoints | Manifest version 1 with generation, server authority, rename dependencies and settled upload IDs | Select the newest valid main/tmp generation; authority mismatch or unsupported state stops sync. Reconfiguration verifies recovery copies before invalidating both files. |
| Upload retry journal | Ordered per-operation files in `pending-uploads/`, bound to the server authority | Persist exact bytes and the operation ID before dispatch. Recover receipts before planning; checkpoint settled IDs before pruning. Reconfiguration archives the journal with both checkpoint generations. |
