...
The current recovery CLI supports source schemas 2, 3, 4 and 5, so supported old deployments can be backed up before upgrading. Isolated restoration upgrades schema 2/3/4 to the current schema 5 while preserving the source archive. Unknown schemas are rejected; see the [compatibility matrix](compatibility.md).
...
1. Deploy the matching Worker version against the restored resources in a new Worker/DO namespace. Use the current Worker with the schema-5 target produced by these tools; supported schema-2/3/4 source archives upgrade during isolated preparation. Unsupported archives are rejected before remote mutation.
