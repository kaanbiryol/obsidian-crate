...
The current recovery CLI supports source schemas 2, 3, 4, 5 and 6, so supported old deployments can be backed up before upgrading. Isolated restoration upgrades schema 2/3/4/5 to the current schema 6 while preserving the source archive. Unknown schemas are rejected; see the [compatibility matrix](compatibility.md).
...
1. Deploy the matching Worker version against the restored resources in a new Worker/DO namespace. Use the current Worker with the schema-6 target produced by these tools; supported schema-2/3/4/5 source archives upgrade during isolated preparation. Unsupported archives are rejected before remote mutation.
