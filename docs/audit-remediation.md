...
| F13 | Converge confirmed reminder state across visible tabs | Fixed: scoped confirmation before queue removal, revision/order guards and fresh-read fallback preserve confirmed state across visible tabs;222 PWA tests and Chromium/WebKit three-tab regressions pass with either sender, stale reads and missed confirmations; lint and typechecks pass; [behavior and test scope](pwa-tab-convergence.md) |
| F14 | Refresh time-dependent views at clock/day/timezone boundaries | Fixed: shared clock drives plugin/PWA headers, Today/Upcoming, cards and embedded lists at minute/deadline/resume boundaries; timezone cache resets on change;708 reminder/PWA tests and Chromium/WebKit native304/midnight/deadline regressions pass, including six DST/timezone/cleanup tests; lint/typecheck pass; [clock behavior](reminder-clock.md) |
...
| S08 | Make sync status keyboard accessible and clarify recurring completion undo | Fixed: keyboard status action previously verified in Chromium/WebKit; recurring actions now name the displayed occurrence, preserve its date when reopened and decrement its count exactly once before recompletion. 25 focused shared/PWA tests and six real Worker tests pass, including repeated lost-response retries against D1/R2; lint and typecheck pass. README states the absence of earlier-occurrence history |
