...
For iOS 27, after building the plugin, run `npm run build:reading-shortcut -- --first-run-setup` on macOS. This writes `dist/Save to Crate (iOS 27).shortcut` and its readable `.plist`. Only the credential-free template is sent to Apple's signing service; credentials are entered afterward. The builder is `scripts/reading-shortcut-first-run.mjs`, using the capture actions from `docs/shortcuts/save-to-crate.plist`. See [Apple's signing documentation](https://support.apple.com/guide/shortcuts-mac/apd455c82f02/mac).

1. In Crate's Reading settings, select **Set up shortcut → Create shortcut access**.
2. Transfer **Save to Crate (iOS 27).shortcut** to the iPhone, open it in Shortcuts, and select **Add Shortcut**. Installation has no setup questions. Run it once from **All Shortcuts**, then paste the endpoint and complete `Bearer …` header when prompted. Wait for **Crate setup saved**. The server must be reachable over HTTPS.
...
The iOS 27 variant uses Apple's shortcut-scoped Storage to remember setup between runs. These values may sync to your other Apple devices through Shortcuts; they are accessible to the shortcut's owner. See [Apple's Storage overview](https://developer.apple.com/videos/play/wwdc2026/310/). Running without a shared URL opens setup again. Cancelling or entering invalid values leaves the previous stored configuration intact; setup checks format, not server authorization. A first share before configuration also opens setup.

The shortcut requires connectivity. Use the enrolled web app's link form for an offline pending save. Capture and browser credentials expire after 90 days and can be revoked from connected devices in Obsidian. A changed tunnel hostname needs fresh browser enrollment and an updated shortcut endpoint. Run the iOS 27 shortcut from its library to update either setup value.
