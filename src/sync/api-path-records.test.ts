...
const entry: FileEntry = { hash: 'a'.repeat(64), size: 1, modified: '2026-01-01', revision: 'r1' };
const noChanges = { changes: [], lastSeq: 3, snapshotSeq: 3, hasMore: false, cursorExpired: false };
...
describe('literal path names in sync API records', () => {
	it('merges paginated manifests and changelog puts/deletes without losing prototype names', async () => {
		const { client, transport } = clientWithResponses(
			...
			Response.json({ version: 1, files: { constructor: entry, toString: entry }, lastSeq: 3, snapshotSeq: 3, hasMore: false, cursorExpired: false }),
			Response.json({ changes: [
				{ seq: 4, path: '__proto__', action: 'put', hash: 'e'.repeat(64), size: 2, created_at: '2026-01-02', revision: 'r2' },
				{ seq: 5, path: 'constructor', action: 'delete', hash: entry.hash, size: 1, created_at: '2026-01-02' },
			], lastSeq: 5, hasMore: false, cursorExpired: false }),
		);
		const manifest = await client.getManifest();
		expect(Object.keys(manifest.files)).toEqual(['__proto__', 'toString']);
		expect(manifest.files.__proto__).toEqual({ hash: 'e'.repeat(64), size: 2, modified: '2026-01-02', revision: 'r2' });
	...
	it('preserves names when an older server falls back from metadata to a manifest', async () => {
		const { client } = clientWithResponses(
			...
			Response.json({ version: 1, files: { ['__proto__']: entry }, lastSeq: 3, snapshotSeq: 3, hasMore: false, cursorExpired: false }),
