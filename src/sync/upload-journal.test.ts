...
function fixture() {
	...
	const file = { path: 'note.md', content: btoa('original'), hash: 'a'.repeat(64), size: 8, contentType: 'text/markdown', expectedHash: null };
...
it('preserves settled receipts while promoting a newer temporary checkpoint across repeated crashes', async () => {
	...
	manifest.setEntry(file.path, { hash: file.hash, size: file.size, revision: 'revision', modified: 'now' });
...
it('drops already-pruned receipt IDs on restart instead of accumulating them forever', async () => {
	...
	reopened.setEntry(file.path, { ...reopened.getEntry(file.path)!, modified: 'later' });
	await reopened.save();
