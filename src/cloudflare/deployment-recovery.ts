...
const confirmedSteps = new Set(['create-file-bucket', 'apply-schema', 'create-server-address', 'upload-worker', 'configure-maintenance', 'enable-server-address']);
...
export async function recoverDeployment(api: RecoveryApi, target: CloudflareDeploymentMetadata): Promise<DeploymentRecoveryResult> {
    ...
    const settings = await api.getWorkerSettings(account, target.workerName);
    const db = settings.bindings?.find(binding => binding.name === 'DB');
    const bucket = settings.bindings?.find(binding => binding.name === 'BUCKET');
    if (db?.type !== 'd1' || db.id !== database || bucket?.type !== 'r2_bucket' || bucket.bucket_name !== target.r2BucketName) {
    ...
    if (!build) return result('blocked', 'The live server could not be identified as a supported Crate deployment.');
