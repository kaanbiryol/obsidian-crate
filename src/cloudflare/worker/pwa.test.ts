...
describe('PWA activation metadata', () => {
	...
	it('keeps standalone safe areas outside visible navigation chrome', () => {
		...
		expect(html).toContain('.pwa-reminders-view .bottom-tab-bar{width:100%;max-width:none;height:var(--reminders-tabbar-height);overflow:hidden;padding-bottom:0}');
		...
		expect(html).toContain('bottom:calc(var(--reminders-tabbar-height) + var(--reminders-fab-gap))');
