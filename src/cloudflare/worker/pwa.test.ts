...
describe('PWA activation metadata', () => {
	...
	it('uses a restrained violet for every PWA interaction accent', () => {
		...
		expect(html).toContain('.crate-reminders-ui .reminders-view:is(.is-inbox, .is-today, .is-upcoming, .is-browse) .reminders-fab {');
	...
	it('uses native-feeling sheet and list interaction affordances', () => {
		...
		expect(html).toContain('.crate-reminders-ui .reminders-view:is(.is-inbox, .is-today, .is-upcoming, .is-browse) [data-reminder-section=completed] {');
