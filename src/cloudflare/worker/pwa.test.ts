...
describe('PWA activation metadata', () => {
	...
	it('keeps standalone safe areas outside visible navigation chrome', () => {
		...
		expect(html).toContain(':root{--pwa-launch-bg:#0d0d0f;color-scheme:dark}');
		expect(html).toContain('html,body,#app{background:var(--pwa-launch-bg);color-scheme:inherit}');
		expect(html).toContain('@media (prefers-color-scheme: light){:root{--pwa-launch-bg:#f7f7f8;color-scheme:light}}');
		...
		expect(html.indexOf('<script nonce=')).toBeLessThan(html.indexOf('<body>'));
		expect(html.indexOf('<script nonce=')).toBeLessThan(html.indexOf('<div id="app"><div class="pwa-launch-splash"'));
