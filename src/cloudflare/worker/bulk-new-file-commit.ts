...
export async function commitNewFiles(bucket: R2Bucket, db: D1Database, files: NewFileUpload[]): Promise<UploadResult[]> {
  ...
  const json = JSON.stringify(files.map(file => ({ path: file.path, hash: file.hash, size: file.size, key: file.objectKey })));
  await db.batch([
    ...files.map(file => uploadMutation(db, file.path, file.hash, file.size, file.objectKey, null, file.operation)),
    db.prepare(`INSERT INTO changelog(path, action, hash, size, revision)
      ...
      ON f.path = json_extract(i.value, '$.path') AND f.storage_key = json_extract(i.value, '$.key')`).bind(json),
    ...bulkFileProjection(db, files),
  ...
  await deleteBucketObjectsOrQueue(bucket, db, unused);
  return results;
}
