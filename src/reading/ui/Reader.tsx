...
export function ReadingReader({ item, markdown, onBack, onEdit, onUpdate, status, onRetry, notice, mutationPending = false, loadingError, onRetryOpen }: {
	...
	status?: string; onRetry?: () => Promise<void>; notice?: React.ReactNode; mutationPending?: boolean;
}) {
	...
	return <article ref={article} className="crate-reading crate-reading-reader" data-serif={serif} style={{ '--reading-font-size': `${fontSize}px` } as React.CSSProperties}>
		<nav className="crate-reading-reader__nav" aria-label="Article actions">
			...
			<div className="crate-reading-reader__mode"><span>Reader</span><a href={item.source_url} target="_blank" rel="noopener noreferrer">Original<ThemeIcon id="arrow-up-right" size="xs" aria-hidden="true" /></a></div>
			<div className="crate-reading-reader__quick-actions">{onUpdate && <><IconButton size="large" icon={item.reading_status === 'archived' ? 'archive-restore' : 'archive'} label={item.reading_status === 'archived' ? 'Move to inbox' : 'Archive article'} disabled={busy || mutationPending} onClick={() => void run(() => onUpdate({ reading_status: item.reading_status === 'archived' ? 'inbox' : 'archived' }))} /><IconButton size="large" icon="star" label={item.favorite ? 'Remove favorite' : 'Favorite article'} aria-pressed={item.favorite} data-filled={item.favorite} disabled={busy || mutationPending} onClick={() => void run(() => onUpdate({ favorite: !item.favorite }))} /></>}</div>
		</nav>
		<div className="crate-reading-reader__page">
			{notice}
			<header className="crate-reading-reader__header"><a className="crate-reading-reader__source" href={item.source_url} target="_blank" rel="noopener noreferrer"><ReadingSourceIcon item={item} />{readingSource(item.source_url)}<ThemeIcon id="arrow-up-right" size="xs" aria-hidden="true" /></a><h1 ref={heading} tabIndex={-1}>{item.title}</h1>
				...
				<div className="crate-reading-reader__tools"><div className="crate-reading-reader__availability">{status && <span role="status"><ThemeIcon id="check" size="xs" aria-hidden="true" />{status}</span>}</div><IconButton size="large" icon="type" label="Reading appearance" onClick={() => setDialog('appearance')} />{onUpdate && <IconButton size="large" icon="hash" label="Edit article tags" disabled={mutationPending} onClick={() => { setTags(item.tags.join(', ')); setDialog('tags'); }} />}{onEdit && <IconButton size="large" icon="file-text" label="Open note" onClick={onEdit} />}<IconButton size="large" icon={copied ? 'check' : 'share-2'} label={copied ? 'Link copied' : 'Share article'} disabled={busy} onClick={() => void run(share)} /></div>
			...
			</div> : <div className="crate-reading-reader__body" dangerouslySetInnerHTML={{ __html: html }} />}
			{markdown !== null && item.extraction_status === 'ready' && <footer className="crate-reading-reader__end"><span aria-hidden="true">✦</span><p>You’ve reached the end.</p>{onUpdate && item.reading_status === 'inbox' && <Button variant="outline" disabled={busy || mutationPending} onClick={() => void run(() => onUpdate({ reading_status: 'archived' }))}><ThemeIcon id="archive" size="m" aria-hidden="true" />Mark as read</Button>}</footer>}
		</div>
		...
		{dialog === 'tags' && onUpdate && <ReadingDialog title="Article tags" busy={busy} onClose={() => setDialog(null)}>{close => <form className="crate-reading__capture" onSubmit={event => { event.preventDefault(); void run(async () => { await onUpdate({ tags: [...new Set(tags.split(',').map(tag => tag.trim()).filter(Boolean))] }); close(); }); }}><label>Tags, separated by commas<input data-initial-focus value={tags} disabled={busy} placeholder="design, essays, inspiration" onChange={event => setTags(event.target.value)} /></label>{error && <p role="alert">{error}</p>}<div className="crate-reading-dialog__actions"><Button variant="outline" disabled={busy} onClick={close}>Cancel</Button><Button variant="primary" type="submit" disabled={busy}>Save tags</Button></div></form>}</ReadingDialog>}
	</article>;
}
